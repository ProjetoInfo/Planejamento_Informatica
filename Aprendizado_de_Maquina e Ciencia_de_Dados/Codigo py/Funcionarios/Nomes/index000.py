...
def selecionar_aleatorio(num_linhas=30, sep=','):
    """
    ...
    try:
        ...
        if len(dados) < num_linhas:
            ...
            amostra = dados.sample(n=num_linhas, random_state=30)
...
amostra_selecionada = selecionar_aleatorio(num_linhas=30)

# Salva a amostra em um novo arquivo (opcional)
if amostra_selecionada is not None:
    amostra_selecionada.to_csv('name_employee.csv', index=False)
    print("Amostra salva no arquivo 'name_employee.csv'.")